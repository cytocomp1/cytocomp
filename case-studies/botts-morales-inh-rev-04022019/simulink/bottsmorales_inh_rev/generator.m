sys = 'bottsmorales_inh_rev';
new_system(sys);
open_system(sys);
% Block 'Block E + S <> ES (kf = 10.0, kr = 1.0)'
...
% Block 'Block ES <> E + P (kf = 20.0, kr = 10.0) (for E)'
add_block('Chip_Library/Aug15_Jon1',[sys '/block3'],'Position',[70.0, 630.0, 310.0, 1050.0]);
set_param([sys '/block3'],'KDfw','0.05');
set_param([sys '/block3'],'KDrv','0.1');
...
% Block 'Block ES <> E + P (kf = 20.0, kr = 10.0) (for P)'
add_block('Chip_Library/Aug15_Jon1',[sys '/block4'],'Position',[450.0, 630.0, 690.0, 1050.0]);
set_param([sys '/block4'],'KDfw','0.05');
set_param([sys '/block4'],'KDrv','0.1');
...
% Incoming connections for block 'E + S <> ES (kf = 10.0, kr = 1.0)'
...
% Incoming connections for block 'E + I <> EI (kf = 5.0, kr = 1.0)'
...
set_param([sys '/block2_Btot'],'Value','50.0');
...
% Incoming connections for block 'ES <> E + P (kf = 20.0, kr = 10.0) (for E)'
...
add_line(sys, 'block1/1','block3/3','autorouting','on');
add_line(sys, 'block4/11','block3/4','autorouting','on');

% Incoming connections for block 'ES <> E + P (kf = 20.0, kr = 10.0) (for P)'
...
set_param([sys '/block1'],'MaskDisplay','ud = get_param([''bottsmorales_inh_rev'' ''/block1''], ''UserData''); image(ud.img);');
...
set_param([sys '/block2'],'MaskDisplay','ud = get_param([''bottsmorales_inh_rev'' ''/block2''], ''UserData''); image(ud.img);');
...
set_param([sys '/block3'],'MaskDisplay','ud = get_param([''bottsmorales_inh_rev'' ''/block3''], ''UserData''); image(ud.img);');
...
set_param([sys '/block4'],'MaskDisplay','ud = get_param([''bottsmorales_inh_rev'' ''/block4''], ''UserData''); image(ud.img);');
