...
% Incoming connections for block 'S + T <> U (kf = 5.0, kr = 5.0)'
...
set_param([sys '/block1_Atot'],'Value','50.0');
...
set_param([sys '/block1_Btot'],'Value','25.0');
...
% Incoming connections for block 'S + V <> W (kf = 10.0, kr = 2.0)'
...
set_param([sys '/outputscope'],'NumInputPorts','5');

% Output block 'S + T <> U (kf = 5.0, kr = 5.0)'
...
% Output block 'S + V <> W (kf = 10.0, kr = 2.0)'
add_line(sys, 'block2/3','outputscope/4','autorouting','on');
add_line(sys, 'block2/11','outputscope/5','autorouting','on');
...
set_param([sys '/block1'],'MaskDisplay','ud = get_param([''fanout'' ''/S + T <> U (kf = 5.0, kr = 5.0)''], ''UserData''); image(ud.img);');
...
set_param([sys '/block2'],'MaskDisplay','ud = get_param([''fanout'' ''/S + V <> W (kf = 10.0, kr = 2.0)''], ''UserData''); image(ud.img);');
set_param([sys '/block2'], 'MaskIconOpaque', 'opaque-with-ports');

% Rename blocks
set_param([sys '/block1'],'Name','S + T <> U (kf = 5.0, kr = 5.0)');
set_param([sys '/block2'],'Name','S + V <> W (kf = 10.0, kr = 2.0)');
